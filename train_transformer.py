...
from utils.utils import set_seed , plot_loss, warmup_cosine
...
def parse_args():
    ...
    parser.add_argument("--val_data_path", type=str, default="./data/val_data.csv", help="Path to val data")
...
def train(model, dataloader, criterion, optimizer, epoch_info, device):
    ...
    for src, tgt in tqdm(dataloader, desc=f"Epoch [{epoch_info[0]+1}/{epoch_info[1]}] Training", unit="batch"):
        ...
        loss = criterion(output, tgt[:, :, -1])  # 假设tgt的最后一列是目标值
        loss.backward()
        optimizer.step()
...
def main():
    ...
    val_dataset = TimeSeriesDataset(
        ...
        # target_scaler=train_dataset.feature_scaler,    
        is_training=False  
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_loss < best_val_loss:    
            ...
            model_save_path = f"{args.save_path}transformer_model_best_{args.model_suffix}.pth"
    ...
    plt_save_path = f"{args.save_path}transformer_model_{args.model_suffix}.png"
